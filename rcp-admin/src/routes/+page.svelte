<script lang="ts">
  ...
  async function login() {
    try {
      const result = await invoke<{success: boolean}>('admin_login', { username, password });
      if (result.success) {
        authenticated = true;
        await loadDashboard();
      }
  ...
  async function loadDashboard() {
    try {
      ...
      const [apps, usersList, stats] = await Promise.all([
        invoke<VirtualApp[]>('get_configured_apps'),
        invoke<User[]>('get_users'),
        invoke<Partial<ServerStatus>>('get_server_stats')
      ]);
  ...
  async function toggleServer() {
    try {
      if (serverStatus.running) {
        ...
        setTimeout(async () => {
          const stats = await invoke<Partial<ServerStatus>>('get_server_stats');
          serverStatus = { ...serverStatus, ...stats };
        }, 1000);
      }
    } catch (e: any) {
  ...
  async function addApplication() {
    try {
      await invoke<void>('add_application', { 
        app: {
          name: newApp.name,
          path: newApp.path,
      ...
      // Reload apps list
      virtualApps = await invoke<VirtualApp[]>('get_configured_apps');
    } catch (e: any) {
      error = e.message;
    }
  }
  ...
  async function toggleAppStatus(appId: string) {
    try {
      await invoke<void>('toggle_app', { appId });
  ...
  async function addUser() {
    try {
      await invoke<void>('add_user', newUser);
  ...
  async function toggleUserActive(userId: string) {
    try {
      await invoke<void>('toggle_user', { userId });
  ...
  function addLogEntry(log: LogEntry): void {
