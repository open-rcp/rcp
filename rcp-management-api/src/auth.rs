...
impl Claims {
    pub fn new(user: &User, expiration_hours: i64) -> ApiResult<Self> {
        let user_id = user.get_id().ok_or_else(|| ApiError::Internal("User has no ID".to_string()))?;
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        ...
        let TypedHeader(Authorization(bearer)) = 
            TypedHeader::<Authorization<Bearer>>::from_request_parts(parts, _state)
                .await
                .map_err(|_| ApiError::Authentication("Missing or invalid authorization header".to_string()))?;
