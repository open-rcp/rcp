...
impl ServiceClient {
    ...
    pub async fn launch_app(&self, app_id: &str, user_id: Option<&str>, args: Option<Vec<String>>) 
    ...
    async fn send_request(&self, request: String) -> Result<serde_json::Value, CliError> {
        ...
        let mut stream = match stream_result {
            Ok(Ok(stream)) => stream,
            Ok(Err(e)) => return Err(CliError::ConnectionError(e.to_string())),
            Err(_) => return Err(CliError::Timeout(self.timeout_seconds)),
        };
        ...
        if let Some(error) = response.get("error") {
            ...
            return Err(CliError::ServerError(error_msg.to_string()));
        }
        ...
        if let Some(result) = response.get("result") {
            ...
            Err(CliError::ServerError("Invalid response format".to_string()))
        }
    }
}
