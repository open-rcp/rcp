...
pub async fn handle_system_diag(formatter: &OutputFormatter) -> Result<()> {
    ...
    if formatter.json_output {
        ...
        data.insert("operating_system", os_info);
        data.insert("memory", memory_info);
        data.insert("disk", disk_info);
        formatter.json(&data).unwrap_or_else(|e| formatter.error(&format!("Failed to format diagnostics data: {}", e)));
    } else {
...
#[cfg(feature = "cli")]
pub async fn handle_network_diag(client: &ServiceClient, formatter: &OutputFormatter) -> Result<()> {
    ...
    if formatter.json_output {
        let mut data = std::collections::HashMap::new();
        data.insert("interfaces", network_interfaces());
        let mut connectivity = std::collections::HashMap::new();
        connectivity.insert("service_reachable", service_check);
        data.insert("connectivity", connectivity);
        formatter.json(&data).unwrap_or_else(|e| formatter.error(&format!("Failed to format network data: {}", e)));
    } else {
...
pub async fn handle_logs(lines: usize, follow: bool, formatter: &OutputFormatter) -> Result<()> {
    // Format logs
    if formatter.json_output {
        formatter.json(&logs).unwrap_or_else(|e| formatter.error(&format!("Failed to format logs: {}", e)));
    } else {
        ...
        if follow {
            ...
            let follow_logs = vec![
                format!("{} INFO  rcpd: New client connection from 192.168.1.105", timestamp()),
                format!("{} DEBUG rcpd: Authentication successful for user 'test'", timestamp()),
                format!("{} INFO  rcpd: Session started for user 'test'", timestamp()),
            ];
...
async fn get_logs(lines: usize) -> Result<Vec<String>> {
    ...
    for i in 0..lines.min(10) {
        ...
        logs.push(format!("{} {} rcpd: {}", formatted_time, level, message));
    }
