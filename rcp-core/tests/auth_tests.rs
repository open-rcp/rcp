...
fn test_session_id_validation() {
    ...
    assert!(!session.session_id.is_nil(), "Session ID should be a valid non-nil UUID");
    ...
    assert!(session.expires_at > now, "Session should not be expired immediately after creation");
}
...
fn test_session_permission_check() {
    ...
    assert!(has_permission(&session, "app.launch"), "Session should have app.launch permission");
    assert!(has_permission(&session, "file.read"), "Session should have file.read permission");
    assert!(!has_permission(&session, "admin.config"), "Session should not have admin.config permission");
}
