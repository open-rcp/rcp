...
/// Trait for RCP services that can be subscribed to by clients
#[async_trait]
...
pub mod builtin {
    ...
    impl Service for DisplayService {
        ...
        async fn start(&mut self) -> Result<()> {
            if self.running {
                return Err(Error::Service("Display service already running".to_string()));
            }
            ...
            tokio::spawn(async move {
                loop {
                    ...
                    let dummy_data = r#"{"width":1280,"height":720,"format":"jpeg"}"#.as_bytes().to_vec();
        ...
        async fn process_frame(&mut self, frame: &Frame) -> Result<()> {
            ...
            match frame.command_id() {
                cmd if cmd == CommandId::VideoQuality as u8 => {
                    // TODO: Process video quality settings
                }
                _ => {
                    return Err(Error::Service(format!(
                        "Unsupported command for display service: {:02x}",
                        frame.command_id()
                    )));
                }
            }
        ...
        async fn get_frame(&mut self) -> Result<Option<Frame>> {
            ...
            match self.frame_receiver.try_recv() {
                ...
                Err(mpsc::error::TryRecvError::Disconnected) => {
                    Err(Error::Service("Display service channel disconnected".to_string()))
                }
            }
        }
    }
        ...
        async fn process_frame(&mut self, frame: &Frame) -> Result<()> {
            ...
            match frame.command_id() {
                cmd if cmd == CommandId::SendInput as u8 => {
                    ...
                    log::debug!("Received input event, {} bytes", frame.payload().len());
                }
                _ => {
                    return Err(Error::Service(format!(
                        "Unsupported command for input service: {:02x}",
                        frame.command_id()
                    )));
                }
            }
    ...
    impl Service for ClipboardService {
        ...
        async fn start(&mut self) -> Result<()> {
            if self.running {
                return Err(Error::Service("Clipboard service already running".to_string()));
            }
        ...
        async fn process_frame(&mut self, frame: &Frame) -> Result<()> {
            ...
            match frame.command_id() {
                cmd if cmd == CommandId::ClipboardData as u8 => {
                    ...
                    log::debug!("Received clipboard data, {} bytes", frame.payload().len());
                }
                _ => {
                    return Err(Error::Service(format!(
                        "Unsupported command for clipboard service: {:02x}",
                        frame.command_id()
                    )));
                }
            }
...
impl ServiceFactory {
    ...
    pub fn create(service_name: &str) -> Option<Box<dyn Service + Send>> {
        match service_name {
            "display" => Some(Box::new(builtin::DisplayService::new())),
            "input" => Some(Box::new(builtin::InputService::new())),
            "clipboard" => Some(Box::new(builtin::ClipboardService::new())),
            _ => None,
        }
    }
