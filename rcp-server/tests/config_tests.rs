...
fn test_default_config() {
    ...
    assert_eq!(config.port, DEFAULT_PORT, "Default port should match DEFAULT_PORT constant");
...
fn test_server_addr_formatting() {
    ...
    assert_eq!(addr, "127.0.0.1:8080", "Server address should be formatted as address:port");
    
    // Verify it can be parsed as a SocketAddr
    let socket_addr = SocketAddr::from_str(&addr);
    assert!(socket_addr.is_ok(), "Server address should be a valid socket address");
    
    let socket_addr = socket_addr.unwrap();
    assert_eq!(socket_addr.port(), 8080, "Port should be 8080");
    assert_eq!(socket_addr.ip().to_string(), "127.0.0.1", "IP should be 127.0.0.1");
}
...
fn test_config_load() {
    ...
    let addr = config.server_addr();
    assert_eq!(addr, "192.168.1.1:9000", "Server address should be formatted correctly");
}
